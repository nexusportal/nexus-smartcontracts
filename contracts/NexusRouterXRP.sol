...
// This router is specifically designed for the XRP EVM, where the native token (XRP)
// is also the wrapped token (WXRP) at address 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE.
// It handles ETH/XRP directly without deposit/withdraw calls.

contract NexusRouterXRP is INexusSwapRouter02 {
    using SafeMathUniswap for uint;

    address public immutable factory;
    address public immutable WETH; // WETH is used as a placeholder for the native XRP address
        ...
        returns (uint amountToken, uint amountETH, uint liquidity)
    {
        ...
        // XRP EVM: Transfer XRP as ERC-20 token instead of sending native currency
        ...
        returns (uint amountToken, uint amountETH)
    {
        ...
        // On XRP EVM, we transfer native XRP directly, no WETH withdraw needed
        ...
        returns (uint[] memory amounts)
    {
        ...
        // XRP EVM: Transfer XRP as ERC-20 token instead of sending native currency
        assert(IERC20(WETH).transfer(NexusSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        // XRP EVM: Transfer XRP as ERC-20 token instead of sending native currency
        assert(IERC20(WETH).transfer(NexusSwapLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20Uniswap(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20Uniswap(path[path.length - 1]).balanceOf(to).sub(
